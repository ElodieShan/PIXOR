...
def axisEqual3D(ax, zoom=1.0):
    extents = np.array([getattr(ax, 'get_{}lim'.format(dim))() for dim in 'xyz'])
    sz = extents[:,1] - extents[:,0]
    centers = np.mean(extents, axis=1)
    maxsize = max(abs(sz)) * zoom
    r = maxsize/2
    for ctr, dim in zip(centers, 'xyz'):
        getattr(ax, 'set_{}lim'.format(dim))(ctr - r, ctr + r)


def plot_scene(data_dict, pred_dict, th=0.5, lab=1, if_gt=True, if_pred=True, scale=0.5):
    scene = plot_points(data_dict['points'], color='w', psize=0.01)

    if if_gt:
        ind = data_dict['gt_boxes'][..., -1] == lab
        bbox = data_dict['gt_boxes'][ind][..., :7]
        scene = plot_bbox(scene, bbox, color=[1, 0, 0])

    if if_pred:
        ind = torch.logical_and(pred_dict['pred_labels'] == lab, pred_dict['pred_scores']>th)
        bbox = pred_dict['pred_boxes'][ind]
        scene = plot_bbox(scene, bbox, color=[0, 1, 0])

    axisEqual3D(scene, scale)
    return scene

def plot_scene_PIXOR(data_dict, pred_dict, th=0.5, lab=1, if_gt=True, if_pred=True, scale=0.5):
    scene = plot_points(data_dict['points'], color='w', psize=0.01)

    if if_gt:
        # ind = data_dict['gt_boxes'][..., -1] == lab
        # bbox = data_dict['gt_boxes'][ind][..., :7]
        bbox = data_dict['gt_boxes']
        scene = plot_bbox(scene, bbox, color=[1, 0, 0])
...
def plot_points(points, figsize=(5, 5), color=None, psize=1, ax=None, axis='off', facecolor='k', equal_axis=True, title=None):
    if type(points) == list:
        for p in points:
...
def plot_bbox(ax, bbox, linewidth=1, color=[0, 1, 0]):
    ...
    for n in range(len(corners3d)):
        ...
        for k in range(0, 4):
            i, j = k, (k + 1) % 4
            ax.plot3D([b[i, 0], b[j, 0]], [b[i, 1], b[j, 1]], [b[i, 2], b[j, 2]], color=color, linewidth=linewidth)
