...
{	
	...
	inline int getX(float x){
		return (int)(X_SIZE- (x-x_MIN)/x_DIVISION);
	}
    ...
    inline int at3(int a, int b, int c){
        return b * (Y_SIZE * (Z_SIZE+1)) + a * (Z_SIZE+1) + c;
    }    
    ...
    void createTopViewMapsfromPoints(const void * indatav, const void * datapoints, int num)
    {	
        
	    float * data_cube = (float *) indatav;
		float * data = (float *) datapoints;
		// load point cloud
    ...
    void createTopViewDensityMapsfromPoints(const void * indatav, const void * datapoints, int num)
    {	
		...
		for (int32_t i=0; i<num; i++) {
			...
			if (*px > x_MIN && *py > y_MIN && *pz >z_MIN && *px < x_MAX && *py < y_MAX && *pz < z_MAX)
			{
				...
				float X_center = (X_SIZE-X)*x_DIVISION + x_DIVISION/2.0;
				float Y_center = y_MAX - Y*y_DIVISION - y_DIVISION/2.0;
				float Z_center = z_MIN + Z*z_DIVISION + z_DIVISION/2.0;

				//For every point in predefined 3D grid box..... 
                *(data_cube + at3(Y, X, Z)) += (1-fabs(*px-X_center))*(1-fabs(*py-Y_center))*(1-fabs(*pz-Z_center)); 
			    *(data_cube + at3(Y, X, Z_SIZE)) += *pr;
				density_map[at2(Y, X)]++;	// update count#, need to be normalized afterwards	
			}
		    px+=4; py+=4; pz+=4; pr+=4;
